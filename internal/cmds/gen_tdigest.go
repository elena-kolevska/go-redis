...
func (c TdigestAdd) Key(key string) TdigestAddKey {
	if c.ks&NoSlot == NoSlot {
		c.ks = NoSlot | uint16(hashtag.Slot(key))
	} else {
		c.ks = check(c.ks, uint16(hashtag.Slot(key)))
	}
...
func (c TdigestMerge) DestinationKey(destinationKey string) TdigestMergeDestinationKey {
	if c.ks&NoSlot == NoSlot {
		c.ks = NoSlot | uint16(hashtag.Slot(destinationKey))
	} else {
		c.ks = check(c.ks, uint16(hashtag.Slot(destinationKey)))
	}
...
func (c TdigestMergeNumkeys) SourceKey(sourceKey ...string) TdigestMergeSourceKey {
	if c.ks&NoSlot == NoSlot {
		for _, k := range sourceKey {
			c.ks = NoSlot | uint16(hashtag.Slot(k))
			break
		}
	} else {
		for _, k := range sourceKey {
			c.ks = check(c.ks, uint16(hashtag.Slot(k)))
		}
	}
